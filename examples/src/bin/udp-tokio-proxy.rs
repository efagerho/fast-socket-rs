...
async fn run(args: Args) -> Result<(), BoxError> {
    ...
    match args.backend {
        Backend::Os => {
            ...
            common::run_actor_tasks("udp-tokio-proxy os", vec![actor], {
                move |handle, rx, stop, total| proxy_actor(handle, rx, stop, total, upstream)
            })
            .await
        }
        Backend::Xdp => {
            ...
            common::run_actor_tasks("udp-tokio-proxy xdp", actors, {
                move |handle, rx, stop, total| proxy_actor(handle, rx, stop, total, upstream)
            })
            .await
        }
