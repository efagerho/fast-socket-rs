...
For the OS backend, this maps to the operating system's UDP socket API. The
kernel builds and parses IP and UDP headers.

For AF_XDP UDP, `XdpUdpSocket` does that work in the backend. It builds
IPv4/UDP and Ethernet headers on transmit. In live mode it parses Ethernet,
IPv4, and UDP in one pass; in first-pass tests it parses normalized IPv4 UDP
datagrams. Both modes expose only the UDP payload at the `UdpSocket` boundary.
