...
impl<D> XdpIpPacketSocket<D> {
    ...
    fn send_test(
        ...
        for slot in batch.iter_mut() {
            ...
            if tx.packet.len() > self.config.mtu || tx.packet.len() > tx.egress.mtu as usize {
                self.stats.dropped_tx_oversize = self.stats.dropped_tx_oversize.saturating_add(1);
                return Err(SendError {
                    accepted,
    ...
    pub fn push_received_ethernet_frame(&mut self, frame: &[u8]) -> bool {
        ...
        if buffer.extend_from_slice(frame).is_err() {
            self.stats.dropped_rx_oversize = self.stats.dropped_rx_oversize.saturating_add(1);
            return false;
        }
            ...
            for slot in batch.iter_mut().take(limit) {
                ...
                if let Err(kind) = validate_xdp_ip_transmit(ifindex, queue_id, mtu, tx) {
                    if matches!(&kind, Error::OversizeForMtu) {
                        self.stats.dropped_tx_oversize =
                            self.stats.dropped_tx_oversize.saturating_add(1);
                    }
    ...
    fn send_udp_inner(
...

        {
            ...
            for slot in batch.iter_mut().take(limit) {
                ...
                let Some(resolved) = router.resolve_udp_l2(*destination.ip(), route_context) else {
    ...
    R: XdpUdpRouter,
{
    ...
    /// builds do not check. See [`XdpPacketBufMut`].
    fn recv(
        &mut self,
