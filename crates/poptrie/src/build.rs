    ...
    V: Clone + Eq + Hash,
{
    ...
    /// Drops interned values no longer referenced by the binary trie and
    /// rewrites node value ids to a dense 1-based table.
    fn compact_live_values(mut bin: Vec<BinNode>, values: Vec<V>) -> (Vec<BinNode>, Vec<V>) {
        let mut remap = vec![0u32; values.len() + 1];
        let mut live = Vec::new();

        for node in &mut bin {
            let old = node.value as usize;
            if old == 0 {
                continue;
            }

            if remap[old] == 0 {
                let new = live.len() as u32 + 1;
                remap[old] = new;
                live.push(values[old - 1].clone());
            }
            node.value = remap[old];
        }
    ...
    pub fn build(self) -> Poptrie<K, V, DIRECT_BITS> {
        ...
        let (bin, values) = Self::compact_live_values(self.bin, self.values);

        assert!(
            values.len() < VALUE_MASK as usize,
