...
use fast_socket_rs::{
    QueueId, RecvBatch, TxSlot, UdpReceive, UdpRxBuffer, UdpSocket, UdpTransmit, UdpTxBuffer,
...
/// Spin-wait OS UDP tile over a vector of [`OsUdpSocket`] members.
pub type SpinOsUdpTile<C> = UdpTile<Vec<OsUdpSocket>, Spin, C>;

/// Park/unpark OS UDP tile over a vector of [`OsUdpSocket`] members.
pub type ParkOsUdpTile<C> = UdpTile<Vec<OsUdpSocket>, Park, C>;

/// Builder for the common OS UDP tile shape.
///
/// The builder hides the concrete socket-set type from application code. Use
/// [`Self::build`] for a spin-wait tile or [`Self::build_parked`] for a
/// park/unpark tile.
    ...
    C: IngressClassifier<DefaultOsRecvMeta, DefaultOsRxBuffer>,
{
    ...
    /// Builds a spin-wait tile.
    #[must_use]
    pub fn build(self) -> Arc<SpinOsUdpTile<C>> {
    ...
    /// Builds a park/unpark tile.
    #[must_use]
    pub fn build_parked(self) -> Arc<ParkOsUdpTile<C>> {
        Arc::new(UdpTile::with_config(
            self.factory,
            self.classifier,
            self.lane_count,
