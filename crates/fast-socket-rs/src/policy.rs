...
/// Borrowed readiness handle for reactor integration on Unix platforms.
#[cfg(unix)]
...
pub trait PollDriver {
    ...
    /// Returns a borrowed reactor wake handle when this driver supports one.
...
pub trait ReadinessSource {
    ...
    /// Returns a borrowed wake handle for reactor registration.
    fn wake_handle(&self) -> Option<WakeHandle<'_>>;
}

/// Generic readiness-mode driver backed by a platform or backend event source.
#[derive(Clone, Debug)]
